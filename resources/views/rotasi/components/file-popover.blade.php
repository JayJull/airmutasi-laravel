    ...
    <div class="flex flex-col gap-2">
        @if (isset($file_url))
            <a href="{{ $file_url }}" target="_blank"
                class="bg-black text-center text-white p-2 rounded-lg font-semibold">Lihat Berkas</a>
        @endif
        <div class="flex w-full">
            ...
            <button id="{{ $id }}_url_set" type="button"
                class="bg-black opacity-80 hover:opacity-100 duration-200 text-white p-2 rounded-e-lg font-semibold">Set</button>
        </div>
        <p class="text-center">atau</p>
        <label
            class="bg-black opacity-80 hover:opacity-100 duration-200 text-white px-2 py-1 rounded-md font-medium hover:cursor-pointer text-center">
